...
- **期間指定集計**：任意の開始日〜終了日を指定してまとめて集計（月またぎ対応）
- 日付昇順/降順の切替
- 行削除（Expander 内セレクトボックス）
- **日付一括削除**：入力日間違い時などに1操作で指定日のデータを全削除
- 当月・全期間 CSV ダウンロード
- **🔄 更新ボタン**：キャッシュを手動クリアして Google Sheets から即時再取得
- API 障害時はエラーメッセージを表示して graceful degradation（キャッシュ汚染防止）
